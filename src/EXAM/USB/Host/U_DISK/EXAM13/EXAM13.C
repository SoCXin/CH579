	...
	if ( i == ERR_SUCCESS ) {  /* 成功获取上级目录的起始簇号 */
		...
		while(1){							/*下面是读取并分析目录项*/			
		   ...
		   if(k<16){
				    ...
				    i=CH579FileClose();
				    i=CH579FileCreate();					/*疑惑这里要不要恢复到刚进入此函数时的簇号*/
						printf("k<16\r\n");
						return i;		/*创建文件,返回状态*/
		   	}
					...
					for(x=0x31;x!=0x3a;x++){					/*生成短文件名*/
						for(y=0x31;y!=0x3a;y++){
							for(i=0x31;i!=0x3a;i++){
								...
								if(CH579FileOpen()!=ERR_SUCCESS)goto XAA1;  /**/	
							}
						}
...
XAA1:
					...
					i=CH579FileRead();				/*读取一个扇区的数据，取第一个目录项就是刚才创建的短文件名*/								
					...
					mCmdParam.Read.mDataBuffer=DATA_BASE_BUF0;	
					CH579DirtyBuffer();						
					i=CH579FileWrite();				/*读取下一个扇区的数据，取第一个目录项就是刚才创建的短文件名*/					
					CH579DirtyBuffer(  );
        ...
        if ( FoundNewDev || s == ERR_USB_CONNECT )                         // 有新的USB设备插入
        {
            ...
            if ( s == ERR_SUCCESS )
            {
                ...
                {      
                    ...
                    if ( s != ERR_SUCCESS )     printf( "Error: %02x\n", s );
                    else     printf( "Creat end\n" );
                }
            }
        }
