...
*                      (2)、串口1输出监控信息,115200bps,打开或者关闭调试打印信息输出请修改CH57xNET_DBG宏定义;
*                      (3)、本程序用于演示TCP 多客户端通讯（示例为4个），单片机收到数据后，回传给目标IP上位机
*******************************************************************************/
...
UINT8 DESIP[4]  = {192,168,10,107};                                              /* 目的IP地址 */
...
UINT8 SocketId1;                                                                /* 保存socket索引，可以不用定义 */
UINT8 SocketId2;                                                                /* 保存socket索引，可以不用定义 */
UINT8 SocketId3;                                                                /* 保存socket索引，可以不用定义 */
UINT8 SocketId4;

UINT8 Socket1RecvBuf[RECE_BUF_LEN];                                             /* socket1接收缓冲区 */
UINT8 Socket2RecvBuf[RECE_BUF_LEN];                                             /* socket2接收缓冲区 */
UINT8 Socket3RecvBuf[RECE_BUF_LEN];                                             /* socket3接收缓冲区 */
UINT8 Socket4RecvBuf[RECE_BUF_LEN];                                             /* socket4接收缓冲区 */
...
* Description    : 创建第一个TCP Client socket
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH57xNET_CreatTcpSocket1(void)
{
   ...
   TmpSocketInf.RecvStartPoint = (UINT32)Socket1RecvBuf;                         /* 设置接收缓冲区的接收缓冲区 */
   TmpSocketInf.RecvBufLen = RECE_BUF_LEN ;                                     /* 设置接收缓冲区的接收长度 */
   i = CH57xNET_SocketCreat(&SocketId1,&TmpSocketInf);                           /* 创建socket，将返回的socket索引保存在SocketId中 */
   mStopIfError(i);                                                             /* 检查错误 */
#ifdef  KEEPLIVE_ENABLE
   CH57xNET_SocketSetKeepLive( SocketId1, 1 );                                   /* 开启socket的KEEPLIVE功能（V06版本支持） */
#endif
...
/*******************************************************************************
* Function Name  : CH57xNET_CreatTcpSocket
* Description    : 创建第二个TCP Client socket
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH57xNET_CreatTcpSocket2(void)
{
   ...
   memset((void *)&TmpSocketInf,0,sizeof(SOCK_INF));                            /* 库内部会将此变量复制，所以最好将临时变量先全部清零 */
   memcpy((void *)TmpSocketInf.IPAddr,DESIP,4);                                 /* 设置目的IP地址 */
   TmpSocketInf.DesPort = 2000;
   TmpSocketInf.SourPort = 4000;                                                /* 设置源端口 */
   TmpSocketInf.ProtoType = PROTO_TYPE_TCP;                                     /* 设置socekt类型 */
   TmpSocketInf.RecvStartPoint = (UINT32)Socket2RecvBuf;                         /* 设置接收缓冲区的接收缓冲区 */
   TmpSocketInf.RecvBufLen = RECE_BUF_LEN ;                                     /* 设置接收缓冲区的接收长度 */
   i = CH57xNET_SocketCreat(&SocketId2,&TmpSocketInf);                          /* 创建socket，将返回的socket索引保存在SocketId中 */
   mStopIfError(i);                                                             /* 检查错误 */
#ifdef  KEEPLIVE_ENABLE
   CH57xNET_SocketSetKeepLive( SocketId2, 1 );                                  /* 开启socket的KEEPLIVE功能 */
#endif

   i = CH57xNET_SocketConnect(SocketId2);                                       /* TCP连接 */
   mStopIfError(i);                                                             /* 检查错误 */
...
void CH57xNET_CreatTcpSocket3(void)
{
   ...
   memcpy((void *)TmpSocketInf.IPAddr,DESIP,4);                                 /* 设置目的IP地址 */
   TmpSocketInf.DesPort = 2000;
   TmpSocketInf.SourPort = 6000;                                                /* 设置源端口 */
   TmpSocketInf.ProtoType = PROTO_TYPE_TCP;                                     /* 设置socekt类型 */
   TmpSocketInf.RecvStartPoint = (UINT32)Socket3RecvBuf;                         /* 设置接收缓冲区的接收缓冲区 */
   TmpSocketInf.RecvBufLen = RECE_BUF_LEN ;                                     /* 设置接收缓冲区的接收长度 */
   i = CH57xNET_SocketCreat(&SocketId3,&TmpSocketInf);                          /* 创建socket，将返回的socket索引保存在SocketId中 */
   mStopIfError(i);                                                             /* 检查错误 */
#ifdef  KEEPLIVE_ENABLE
   CH57xNET_SocketSetKeepLive( SocketId3, 1 );                                  /* 开启socket的KEEPLIVE功能 */
#endif

   i = CH57xNET_SocketConnect(SocketId3);                                       /* TCP连接 */
   mStopIfError(i);                                                             /* 检查错误 */
...
void CH57xNET_CreatTcpSocket4(void)
{
   ...
   memcpy((void *)TmpSocketInf.IPAddr,DESIP,4);                                 /* 设置目的IP地址 */
   TmpSocketInf.DesPort = 2000;
   TmpSocketInf.SourPort = 8000;                                                /* 设置源端口 */
   TmpSocketInf.ProtoType = PROTO_TYPE_TCP;                                     /* 设置socekt类型 */
   TmpSocketInf.RecvStartPoint = (UINT32)Socket3RecvBuf;                        /* 设置接收缓冲区的接收缓冲区 */
   TmpSocketInf.RecvBufLen = RECE_BUF_LEN ;                                     /* 设置接收缓冲区的接收长度 */
   i = CH57xNET_SocketCreat(&SocketId4,&TmpSocketInf);                          /* 创建socket，将返回的socket索引保存在SocketId中 */
   mStopIfError(i);                                                             /* 检查错误 */
#ifdef  KEEPLIVE_ENABLE
   CH57xNET_SocketSetKeepLive( SocketId4, 1 );                                  /* 开启socket的KEEPLIVE功能 */
#endif
...
void CH57xNET_HandleSockInt(UINT8 sockeid,UINT8 initstat)
{
    ...
    {
#if CH57xNET_DBG                                                                /* 产生此中断表示TCP已经连接，可以进行收发数据 */
        printf("TCP Connect Success\n");
#endif
    }
    if(initstat & SINT_STAT_DISCONNECT)                                         /* TCP断开中断 */
    {
#if CH57xNET_DBG                                                                /* 产生此中断，CH579库内部会将此socket清除，置为关闭*/
...
int main(void)
{
	...
	NVIC_EnableIRQ(ETH_IRQn);                                                   /* 打开以太网中断 */
	while ( CH57xInf.PHYStat < 2 ) DelayMs(50);
    CH57xNET_CreatTcpSocket1();                                                 /* 创建第一个TCP Socket */
	CH57xNET_CreatTcpSocket2();                                                 /* 创建第二个TCP Socket */
    CH57xNET_CreatTcpSocket3();
