...
static void cfg_cli_rsp_handler(const cfg_cli_status_t *val)
{
	...
	switch (val->cfgHdr.opcode)
	{
		...
		case OP_MOD_APP_BIND:
			...
			err = bt_mesh_cfg_mod_sub_add(net_idx, addr, addr, 0xC000, BLE_MESH_MODEL_ID_GEN_ONOFF_SRV);
			if (err)
			{
		...
		case OP_MOD_SUB_ADD:
			BT_DBG("OnOff Model Subscription Seted");
			break;
...
u8C MacAddr[] = {0x12, 0x23, 0x34, 0x45, 0x56, 0x67};

#define LOCAL_ADV_NAME	"WCH-BLE-MESH"

static void lpn_cb(u16_t friend_addr, bool established)
{
	BT_DBG("friend addr 0x%04x estab 0x%02x", friend_addr, established);
}
