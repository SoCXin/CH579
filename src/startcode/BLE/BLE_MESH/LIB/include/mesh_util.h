...
#define SIZES_OF_TYPE(_type, _count) (sizeof(_type) * (_count))
...
static inline unsigned int find_popcount(u32_t bits)
{
    u16_t counts;
    int i;

    for (i = 0, counts = 0; i < 32; i++)
    {
        if (bits & BIT(i))
        {
            counts++;
...
#define popcount(x) find_popcount(x)
 ...
 */
    ...
    uint8_t get_msb_set(uint32_t bits);
    uint8_t get_lsb_set(uint32_t bits);
